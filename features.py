...
def turns_since_move(state, maximum):
...
def get_liberties(state, maximum):
    planes = np.zeros((maximum, state.size, state.size))
    for i in range(maximum):
        x = 0
        y = 0
...
def get_capture_size(state, maximum):
...
def get_self_atari_size(state, maximum):
    planes = np.zeros((maximum, state.size, state.size))
    for (x, y) in state.get_legal_moves():
        for (nx, ny) in self._neighbors((x, y)):
            if state.board[nx, ny] == color and len(state.liberty_sets[nx][ny]) == 0:
                if len(state.group_sets[nx][ny]) < maximum:
                    planes(len(state.group_sets[nx][ny]), x, y) = 1
                else planes(maximum-1, x, y) = 1
    return planes

def get_liberties_after(state, maximum):
    planes = np.zeros((maximum, state.size, state.size))
    x = 0
    y = 0
    while x < size:
